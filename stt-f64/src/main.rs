...
async fn main() -> tokio::io::Result<()> {
    ...
    while let Some(class) = train_dir.next_entry().await? {
        ...
        join_set.spawn(async move {
            if class.file_type().await?.is_dir() {
                ...
                let wav_vec: Vec<Vec<f64>> = wav_list
                    .iter()
                    .map(|wav_path| {
    ...
    if settings.stt_settings.monophone_training.enable {
        ...
        let bigram_log_prob: Array2<f64> = {
    ...
    if settings.stt_settings.predict.enable || settings.stt_settings.predict.real_time {
        let model = {
            info!("Loading model");
            if Path::new(&settings.stt_settings.storage.monophone_modele_file).exists() {
                let mut ron_model =
                ...
                let triphone = ron::from_str::<MonoPhone>(&string_model).unwrap();
                info!("Model loaded");
                Arc::new(triphone) as Arc<dyn ViterbiTrait>
            } else {
                let mut ron_model =
                    File::open(&settings.stt_settings.storage.monophone_modele_file).await?;
                let mut string_model = String::new();
...
fn input_callback(storage_tx: watch::Sender<Vec<f64>>, data: &[f32], _info: &InputCallbackInfo) {
    storage_tx
        .send(data.iter().map(|s| *s as f64).collect())
        .unwrap();
...
async fn output_callback(
    storage: Arc<Mutex<VecDeque<f64>>>,
    data: &mut [f64],
